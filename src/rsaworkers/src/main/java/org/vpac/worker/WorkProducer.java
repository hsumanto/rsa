...
import static org.vpac.worker.Frontend.NotOk;
import static org.vpac.worker.Frontend.Ok;
...
public class WorkProducer extends UntypedActor {
	...
	public void preStart() {
		scheduler.scheduleOnce(Duration.create(5, "seconds"), getSelf(), Tick,
				ec, getSelf());
	}
			...
			public void apply(Object message) {
				if (message instanceof Ok) {
					...
					scheduler.scheduleOnce(
							Duration.create(rnd.nextInt(3, 10), "seconds"),
							getSelf(), Tick, ec, getSelf());
				} else if (message instanceof NotOk) {
					log.info("Work not accepted, retry after a while");
					scheduler.scheduleOnce(Duration.create(3, "seconds"),
							frontend, work, ec, getSelf());
				} else {
					unhandled(message);
				}
			}
