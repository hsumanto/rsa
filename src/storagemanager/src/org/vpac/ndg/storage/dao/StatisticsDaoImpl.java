...
public class StatisticsDaoImpl extends CustomHibernateDaoSupport implements StatisticsDao {
	...
	//
	// @Transactional
	// public List<TimeSlice> findTimeSlices(String datasetId, Date startDate,
	// Date endDate) {
	// Session session = getSession();
	// String queryString =
	// "SELECT s FROM Dataset d JOIN d.slices s WHERE d.id=:id";
	...
	// Session session = getSession();
	// String queryString =
	// "SELECT b FROM Dataset d JOIN d.bands b WHERE d.id=:id AND b.id IN (:bandIds) ORDER BY b.name";
	// Query query = session.createQuery(queryString);
	// query.setParameter("id", datasetId);
	...
	// @Transactional
	// @Override
	// public List<Band> findBandsByName(String datasetId, List<String>
	// bandNames) {
	// Session session = getSession();
	// String queryString =
	// "SELECT b FROM Dataset d JOIN d.bands b WHERE d.id=:id AND b.name IN (:bandNames) ORDER BY b.name";
	// Query query = session.createQuery(queryString);
	// query.setParameter("id", datasetId);
