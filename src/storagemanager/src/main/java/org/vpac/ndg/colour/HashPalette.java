...
public class HashPalette implements Palette {
	...
	public Color get(double value) {
		...
		// The cvalue needs to match the pixel values that would be used in an
		// image. When using glad_translate to rescale an image to an interger
		// type, it uses round, not floor.
		long cvaluei = Math.round(cvalue);
		double hashedValue = hash((int) cvaluei);
